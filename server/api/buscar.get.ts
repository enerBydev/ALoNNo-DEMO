...
async function buscar(event: any) {
	...
	if (q.length > 500)
		throw createError({
			statusCode: 400,
			statusMessage: "frase demasiado larga",
		});
	...
	const ciudadDeQuienPregunta =
		String(getQuery(event).ciudad ?? "").trim() || null;
	...
	if (directo) {
		// EL CAMINO DIRECTO, DE VERDAD. El programa de UX del 15-sep-2026 lo midio: `directo` se
		// calculaba y no se usaba —el bloque que aplicaba los campos nunca llego al fichero—, asi
		// que corregir «hacia Westend» devolvia Mitte, la pantalla decia «sin llamar al modelo» y el
		// modelo se llamaba igual. Tres agentes distintos lo encontraron por separado.
		//
		// Aqui no hay nada que interpretar: la frase la escribio el formulario. La intencion sale
		...
		if (q_cuando) {
			...
			if ((permitidas as readonly string[]).includes(q_cuando)) {
				intencion.fecha = {
					expresion: q_cuando as (typeof permitidas)[number],
					mes: null,
				};
			}
		}
		...
		try {
			...
			motivoDegradado = String(e?.statusMessage ?? e?.message ?? e).slice(
				0,
				200,
			);
			console.warn(
	...
	const [resPersonas, resPlanes] = await Promise.all([
		...
		rpc<Plan>(env, "buscar_planes", {
			...
			// Un trayecto de diario no mide 681 km: para una frase de coche se cortan los viajes
			// largos, que solo «pasaban por ti» porque empiezan en casa del conductor.
			q_max_km: intencion.trayecto ? 120 : null,
			q_excluir: null,
			q_limite: 50,
		}),
	]);
	...
	const tagsConsulta = [
		...intencion.must_match,
		...intencion.nice_to_have,
		intencion.subject ?? "",
		intencion.category ?? "",
	].filter(Boolean);
	...
	const planes = resPlanes.filas.map((p) => ({
		...
		puntuacion: puntuarPlan(p, {
			...
			subject:
				intencion.subject ?? (intencion.trayecto ? intencion.hacia : null),
			subjectEspecificidad:
				intencion.trayecto && intencion.hacia && !intencion.subject
					? "named"
					: intencion.subject_specificity,
	...
	const empuje = (t: "PERSONA" | "PLAN") =>
		MEZCLA[intencion.archetype]?.[t] ?? 1;

	type Fila = ((typeof personas)[number] | (typeof planes)[number]) & {
		orden: number;
	};
	const todos: Fila[] = [...personas, ...planes].map((f) => ({
		...f,
		orden: f.puntuacion.porcentaje * empuje(f.tipo),
	}));
	...
	const porSuNumero = (a: Fila, b: Fila) =>
		b.puntuacion.porcentaje - a.puntuacion.porcentaje;
	const vivosPersonas = todos
		.filter((f) => f.tipo === "PERSONA" && !f.puntuacion.descartado)
		.sort(porSuNumero);
	const vivosPlanes = todos
		.filter((f) => f.tipo === "PLAN" && !f.puntuacion.descartado)
		.sort(porSuNumero);
	const vivos = [...vivosPersonas, ...vivosPlanes].sort(
		(a, b) => b.orden - a.orden,
	);
	const descartados = todos
		.filter((f) => f.puntuacion.descartado)
		.sort(porSuNumero);
	...
	const salida = (f: Fila) => ({
		...
		viaje:
			f.tipo === "PLAN" && (f as any).distancia_km != null
				? {
						...
						precio_total:
							Math.round(
								Number((f as any).distancia_km) *
									Number((f as any).precio_por_km) *
									100,
							) / 100,
						...
						km_de_tu_ruta:
							(f as any).km_ruta != null
								? Math.round(Number((f as any).km_ruta) * 10) / 10
								: null,
			...
			f.tipo === "PLAN"
				? (f as any).conductor
					? {
							...
							nota:
								(f as any).conductor_nota != null
									? Number((f as any).conductor_nota)
									: null,
		...
		plazas_coche:
			f.tipo === "PERSONA" ? ((f as any).plazas_coche ?? null) : null,
	});
	...
	return {
		...
		listas: (mandaElViaje
			? [
					{
						clave: "viajes" as const,
						titulo: intencion.trayecto
							? "Rides going your way"
							: "Plans you could join",
						filas: vivosPlanes,
					},
					{
						clave: "personas" as const,
						titulo: intencion.trayecto
							? "Drivers who match you"
							: "People who fit you",
						filas: vivosPersonas,
					},
				]
			...
			clave: l.clave,
			titulo: l.titulo,
			total: l.filas.length,
			resultados: l.filas.slice(0, 8).map(salida),
		})),
...
function claveDeCache(event: any): string {
	...
	const ciudad = String(g.ciudad ?? "")
		.trim()
		.toLowerCase();
	const campos = [g.desde, g.hacia, g.cuando]
		.map((x) =>
			String(x ?? "")
				.trim()
				.toLowerCase(),
...
// EL ALMACEN ES EL BINDING, NO `useStorage('cache')`. Medido el 15-sep-2026 con
// `wrangler kv key list`: el KV solo tenia la clave del freno —que escribe contra
// `event.context.cloudflare.env.CACHE` a pelo— y NINGUNA entrada de busqueda. El montaje de Nitro
// (`nitro.storage.cache` → `cloudflare-kv-binding`) no resolvia el binding en el Worker y caia a
// memoria por isolate: los HIT eran reales dentro de un isolate y desaparecian al saltar de colo.
// Por eso `just calentar` «funcionaba» y la portada volvia a salir degradada media hora despues.
//
// En desarrollo no hay binding: un Map en memoria hace el mismo papel y se declara.
...
function kvDe(event: any): { get: (k: string) => Promise<any>; put: (k: string, v: any) => Promise<void> } {
	...
	if (kv?.get && kv?.put) {
		return {
			get: (k) => kv.get(k, "json"),
			put: (k, v) => kv.put(k, JSON.stringify(v), { expirationTtl: VIDA_S }),
		};
	}
	return {
		get: async (k) => {
			const e = memoria.get(k);
			return e && e.hasta > Date.now() ? e.valor : null;
		},
		put: async (k, v) => {
			memoria.set(k, { hasta: Date.now() + VIDA_S * 1000, valor: v });
		},
	};
}
...
export default defineEventHandler(async (event) => {
	...
	if (!fresco) {
		const guardado = await almacen.get(clave).catch(() => null);
